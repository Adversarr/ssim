...
if [ -d "mathprim" ]; then
  cd mathprim
  Echo "${INFO_COLOR}Pulling the latest version of mathprim to $(pwd)...${NO_COLOR}"
  git pull &>/dev/null
  if [ $? -ne 0 ]; then
    Echo "${WARNING_COLOR}Failed to pull the latest version of mathprim.${NO_COLOR}"
    exit
  fi
  # show version
  git log -1 --pretty=format:"%h %s"
  Echo "${INFO_COLOR}Git pull finished.${NO_COLOR}"
  cd ..
else
  Echo "${INFO_COLOR}Cloning mathprim...${NO_COLOR}"
...
if [ -n "$(command -v nvcc)" ]; then
  ...
  Echo "${INFO_COLOR}nvcc found. CUDA will be enabled.${NO_COLOR}"
else
  Echo "${INFO_COLOR}nvcc not found. CUDA will be disabled.${NO_COLOR}"
fi

Echo "${INFO_COLOR}Configuring mathprim...${NO_COLOR}"
...
if [ $? -ne 0 ]; then
  Echo "${WARNING_COLOR}Failed to configure mathprim.${NO_COLOR}"
  exit
fi

Echo "${INFO_COLOR}Build&Install mathprim...${NO_COLOR}"
cmake --build _build >/dev/null
if [ $? -ne 0 ]; then
  Echo "${WARNING_COLOR}Failed to build mathprim.${NO_COLOR}"
  exit
fi
cmake --install _build >/dev/null
if [ $? -ne 0 ]; then
  Echo "${WARNING_COLOR}Failed to install mathprim.${NO_COLOR}"
  exit
fi
