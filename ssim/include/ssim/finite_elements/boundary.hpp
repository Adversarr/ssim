...
public:
  ...
  using dof_type_type = mp::contiguous_view<const node_type, mp::shape_t<DofsPerNode>, Device>;
  ...
  using const_batched_value = mp::batched<value_type>;
  ...
  SSIM_PRIMFUNC const const_batched_dof_type& node_type() const noexcept { return node_type_; }
  SSIM_PRIMFUNC const batched_value& value() const noexcept { return value_; }

  struct set_values_impl {
    SSIM_PRIMFUNC void operator()(const vertex_type& vertex,      //
                                  const dof_type_type& dof_type,  //
                                  const value_type& value) const noexcept {
      for (index_t i = 0; i < DofsPerNode; ++i) {
        if (dof_type(i) == node_type::dirichlet) {
          vertex(i) = value(i);
        }
      }
    }
  ...
  struct identity_hessian_impl {
    ...
    SSIM_PRIMFUNC void operator()(index_t i, index_t j, Scalar val) {
      ...
      if (itype[0] == node_type::dirichlet && jtype[0] == node_type::dirichlet) {
        val = i == j ? 1 : 0;
      }
    }
  };
  ...
  void value(mp::par::parfor<ParImpl>& pf, batched_value values) {
    auto set_values = set_values_impl{};
    pf.vmap(set_values, mesh_.vertices(), node_type_, values);
  }

  /// @brief Set the grads of dirichlet boundaries to zero.
  template <typename ParImpl>
  void grads(mp::par::parfor<ParImpl>& pf, batched_value grads) {
  ...
  template <typename ParImpl>
  void hessian(mp::par::parfor<ParImpl>& pf, hessian_type hessian) {
    identity_hessian_impl ijk_task{node_type_};
    pf.run(hessian.visit(ijk_task));
  }
